...
CREATE_WALLET_CMD="bitcoin-cli ${AUTH_ARGS} createwallet ${RPC_USER}"
...
WALLET_INFO=$(docker exec ${CONTAINER_NAME} bash -c "${CREATE_WALLET_CMD}")

# check if there are any errors in the output
if [[ "$WALLET_INFO" == *"Error"* && "$WALLET_INFO" != *"already exists"* ]]; then
